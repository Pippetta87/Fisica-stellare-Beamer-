...
\begin{frame}{Network reazioni nucleari}
L'energia liberata dalle reazioni nucleari per grammo per secondo $\epsilon(\rho,T,X_i)$ \'e determinata dalla probabilit\'a che la reazione $X(a,b)Y$ abbia luogo.
\end{frame}

\begin{frame}{Tempi evoluzione chimica catena PP}

\begin{figure}[!ht]
	\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,width=0.45\textwidth]{ppchainsequi}\label{fig:ppchainsequi}~
		\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,width=0.45\textwidth]{pplifetime}\label{fig:pplifetime}
\end{figure}
\end{frame}
...
\begin{frame}{Sezione d'urto nucleare}
 Sia $E$ l'energia cinetica nel centro di massa dei nuclei, la sezione d'urto $\sigma(E)=\pi\lambdabar^2*P_0(E)*S(E)$
...
per energie tipiche degli interni stellari approx onda S), della probabilit\'a di attraversamento della barriera coulombiana e del fattore astrofisico.
...
\subsection{Reazioni nucleari risonanti (e non)}

\subsection{Produzione energia da H-burning}
