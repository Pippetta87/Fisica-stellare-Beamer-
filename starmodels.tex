...
\begin{frame}{Thermodynamics relation: $dq=du+Pdv=c_PdT-\frac{\delta}{\rho}dP$}
    \begin{columns}[T]
...
\begin{frame}{Total Radiative opacity}
    \begin{itemize}
        \item $\rho\kappa(\nu)=n\sigma(\nu)$
        \item At low T where atoms are part. ionized opacity is dominated by BB, BF by bound electrons. 
        \item As ionization nears completion opacity due to FF becomes dominant but because Rosseland mean  $\kappa_{ff}$ decreases with increasing T there is a Temperature where electron scattering becomes dominant.
        \item All forms of opacity contribute sim.: sum freq dependent opacity, correct true absorption for induced emission, $[\kappa_{bb}(\nu)+\kappa_{bs}(\nu)+\kappa_{ff}(\nu)](1-\exp{-\frac{h\nu}{kT}})+\kappa_s$, then form Rosseland mean. In general $\exv{\kappa_1}+\exv{\kappa_2}=\exv{\kappa_1+\kappa_2}$ (Two Rosseland mean are additive if are true absorption opacity and have same freq. dependence).
        \end{itemize}
\end{frame}

\begin{frame}{Deps of L from composition (using Kramer's opacity)}
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{align*}
...
\begin{itemize}
    \item Electron scattering $\rho\kappa_{\nu}=n_e\frac{8\pi}{3}(\frac{e^2}{m_ec^2})^2=0.2(1+X)\si{\square\cm\per\gram}$ - $\sigma_T=\frac{8\pi}{3}(\frac{e^2}{m_ec^2})^2=\SI{0.66e-24}{\square\cm}$ - for $T$ maggiore di few milion K is dominant source - Compton scattering $h\nu|_M\gtrsim0.1 m_ec^2$, $h\nu|_M\approx4.96kT$: Compton scattering reduces opacity about $20\%$ for $T>\SI{e8}{\kelvin}$
    \begin{align*}
...
\begin{block}{Free-free: radiation boost free-\Pelectron from lower to higher state}
    \begin{itemize}
\item $\tau_{\Pelectron-int}\propto\frac{1}{v}\propto T\expy{-1/2}$: \Pelectron experience sharp acceleration $\approx\delta$ resulting in constant emission frequency, two body ion-\Pelectron encounter $\propto n_in_e$, \Pelectron have thermal distro $\propto\exp{-\epsilon/kT}$: $\rho j_{\nu}=n_en_iT\expy{-1/2}\exp{-\frac{h\nu}{kT}}$ - dalla legge di Kirchhoff, $j_{\nu}=4\pi\kappa_{\nu}^{abs}B_{\nu}(T)$, we have $\kappa_{\nu}^{abs}\propto\rho T\expy{-1/2}\nu\expy{-3}[1-\expy{-h\nu/kT}]$
        \item Fully ionized-$Z_i$ mixture: $\rho\kappa_{\nu}(ff)=\sum_in_{Z_i}ne\sqrt{\frac{2m_e}{3\pi kT}}[\frac{4\pi Z_i^2e^6}{3m_e^2ch\nu^3}]g_{ff}(\nu)(1-\exp{-h\nu/kT})=\num{3.8e22}\overbrace{(X+1)}^{\propto n_e}\overbrace{[X+Y+B]}^{\propto\frac{X}{m_u}+\frac{4Y}{4m_u}+\sum_i\frac{X_iZ_i^2}{A_i}}\ [\si{\cgs}]$
                \item 
\begin{align*}
    &d\sigma_{ff}=\frac{4\pi}{3\sqrt{3}hc}\frac{Z^2e^6}{m_e^2}\frac{1}{v_e\nu^3}n_ef(v_e)g_{ff}(v_e,\nu)\,dv_e: \bar{\sigma}_{ff}=\num{3.69e8}\frac{Z^2n_e\bar{g}_{ff}}{T^{\frac{1}{2}\nu^3}}\si{\square\cm}\propto \frac{Z^2n_e}{\sqrt{KT}}\frac{\bar{g}_{ff}}{\nu^3}\\
    &\kappa_{ff}(\nu)\rho=\sum_A^Nn(A,Z_A)\exv{\sigma_{ff}(Z_A,\nu)}=\sum_A \frac{X_A\rho}{Am_u}\exv{\sigma_{ff}(Z_A,\nu)}
\end{align*}
\item Per ionizzazione totale $n_e\approx \frac{\rho}{m_p}\frac{1+X}{2}$: $\bar{\kappa}_{ff}=\num{4e22}(X+Y)(1+X)\rho T^{-3.5}\si{\cm\per\gram}$
        \end{itemize}
...
\begin{align*}
    &\sigma_{\nu}(Z)=n\expy{-5}\frac{64\pi^4}{3\sqrt{3}}\frac{(Z')^4m_ee\expy{10}g_{bf}}{ch^3(h\nu)^3}:\ h\nu>\chi=\frac{Ze^2}{2a_Z}\\
...
\begin{frame}{Sezione d'urto fusione nucleare}
    \begin{itemize}
        \item $E$ energia cinetica nel CM dei nuclei -  la sezione d'urto di fusione: $\sigma(E)=\pi\lambdabar^2*P_0(E)*S(E)$ prodotto della sezione d'urto geometrica $\pi\lambdabar^2\propto \frac{1}{E}$ (nel riferimento del CM: $\sigma\approx\sum_{l=0}^{\frac{R}{\lambdabar}}(2l+1)\pi\lambdabar^2=\pi(R+\lambdabar)^2$ per energie tipiche degli interni stellari approx onda S), della probabilit\'a di attraversamento della barriera coulombiana $P\propto\exp{-\frac{r_{classic}}{\lambda}}$ e del fattore astrofisico. La lunghezza d'onda di de Broglie relativa delle particelle descrive l'indeterminazione sulla posizione nell'urto di due particelle con momento relativo p $\lambdabar=\frac{\hbar}{p}=\frac{\hbar}{\sqrt{2mE}}$.
\item energia potenziale dovuta all'interazione di due nuclei $Z_1$ e $Z_2$ a distanza r contiene un contributo delle altre cariche presenti nel plasma $U=\frac{Z_1Z_2e^2}{r}+U_s(r_{12})$, energia potenziale non schermata e contributo della nuvola elettronica: $U_s$ aumenta la probabilit\'a di attraversamento della barriera coulombiana. Fattore moltiplicativo: $f=\exp{-\midfrac{U_0}{KT}}$ dove $U_0=U_s(0)$ poich\'e $r\ll r_D$ e considerando solo la correzione al fattore di penetrazione ($E_G\gg U_0$). Per determinare $U_0$ considero l'energia potenziale di $Z_1$ e $Z_2$ a distanza $r$
\begin{equation*}
...
\end{equation*}
            \item Picco di Gamow - Energia di reazione pi\'u probabile:
                \begin{equation*}
                    E_0=(\frac{1}{2}2\pi\eta E^{\frac{1}{2}}KT)^{\frac{2}{3}}=[(\frac{m}{2})^{\frac{1}{2}}\pi \frac{Z_iZ_ke^2KT}{\hbar}]^{\frac{2}{3}}\approx\SI{5.665}{\kilo\ev}W^{\frac{1}{3}}T_7^{\frac{2}{3}}
                \end{equation*}
        \end{itemize}
\end{frame}
...
\begin{frame}{Gravothermal specific heat}
    \begin{itemize}
...
\begin{frame}{Secular stability of nuclear burning}
    \begin{itemize}
...
\begin{column}{0.55\textwidth}
	\begin{figure}[!ht]
	\includegraphics[trim={0cm 0cm 0cm 0cm},clip, keepaspectratio,height=0.28\textheight]{CNO}
...
\begin{columns}[T]
    ...
    \item Reactiona during He-buring ($\alpha+\alpha\leftrightarrow^8Be$, $Q=-\SI{92}{\kilo\ev}$)
	\begin{align*}
        &^4He(\alpha\alpha,\gamma)^{12}C\ Q=\SI{7274.7}{\kilo\ev}, \frac{N_{\alpha}}{3\tau_{3\alpha}}\\
...
&^{24}Mg(\alpha,\gamma)^{28}Si
	\end{align*}
\item Cross-section of $^{12}C(\alpha,\gamma)^{16}O$: i) important for $C/O$ ratio in WD ii) when $^4He$ abundance become low competes with $3\alpha$ - has strong influence on He-burning phase lifetime.
...
\end{align*}
	...
	\begin{column}{0.35\textwidth}
		\begin{figure}[!ht]
			\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,width=0.8\textwidth]{Heburningelems}
			\includegraphics[trim={0cm 0cm 0cm 0cm},clip, keepaspectratio,width=0.8\textwidth]{3alphalevels}
		\end{figure}
	\end{column}
\end{columns}
\end{frame}
