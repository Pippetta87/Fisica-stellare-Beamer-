...
\begin{block}{Core convective instability}
3 breathing pulses are needed to completely exhaust He in core: when $Y_c\approx0.1$ occurs core convective instability - when $^{12}C+\alpha\to^{16}O$ overcome $^{12}C$ production by $3\alpha$ \xaumenta{\kappa} quindi aumenta la dimensione della zona convettiva quindi \xaumenta{Y_c}, \xaumenta{L}(\xaumenta{\epsilon_{He}}), \xaumenta{\nrad} e a catena umenta zona convettiva (\keyword{breathing pulses}: after each pulse star readjust to burn He steadly).
\end{block}
\begin{block}{Effects of BP}
i) Loop in HDR at each pulse ii) He burning time increased iii) $M_{cc}$ increased
\end{block}
\begin{block}{R2 parameter}
R2 is ratio of number of star in AGB to stars in HB (lifetime ratio): $=0.12-0.15$ without BP, $0.8-0.14$ with BP - observed R2 in globular cluster ([36], [44]): BP efficiency very low, BP cloud be due to breakdown in instantaneous mixing cdapprox in stellar models when convection setin in late HB stages
\end{block}
\end{frame}
...
\begin{frame}{instability strip: }
Refs: [61], [79], [80]
region in HDR where stars are subjected to stable radial pulsation: cepheid instability strip (RR Lyrae and classic cepheids: \keyword{Standard candles} for PopI/II stars)
Adiabatic pulsation with period $\Pi\sqrt{\rho}=Q$, slightly deps on M: $\Pi\propto	sqrt{\frac{R}{g_s}}\propto\sqrt{\frac{R^3}{M}}$.
$\kappa$-mechanism: star energy flow fixes the radius - stability implies fariation in energy flux and thus in R - opacity increases during compression/decreases in expansionat same level in stellar envelope: He/H ionization ragions
...
\begin{columns}[T]
	\begin{column}{0.55\textwidth}
		\begin{itemize}
			...
			\item connection between $\Pi$ and other stellar parameter for ab-stars:
		\end{itemize}
	\end{column}
	\begin{column}{0.45\textwidth}
		\begin{figure}[!ht]
