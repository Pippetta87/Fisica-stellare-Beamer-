...
\end{align*}
Matter processed by CN: \xdiminuisce{C}, \xaumenta{N}; by CNO: \diminuisce{C}, \xaumenta{N}, \ximinuisce{O}.
\end{column}\end{columns}
\end{frame} 
...
\subsection{Post-MS: SGB, RGB??}\linkdest{postMS}
%SC [160]

\begin{frame}{Int-massive stars}
    \begin{itemize}
        \item $M_{He,core}$ maggiore del limite SC
        \item SGB: Structure move from blue to red side at almost constant surface L. $\tkh{}\approx\SI{12}{\mega\yr}$ for $3\msun{}$, $\tkh{}\approx\SI{1}{\mega\yr}$ for $6\msun{}$.
...
\begin{frame}{Condizione Core per fusione: virial theorem and degenerate electrons}
    \begin{itemize}
        \item Electron degeneracy: $3(\gamma-1)$ is 2 (NR) or 1 (R) for degenerate \Pelectron; for mixture \Pelectron-ions $\gamma>\frac{4}{3}$. Case NR \Pelectron deg. $3(\gamma-1)=2$  (NR: $\gamma=\frac{5}{3}$): $\Omega=-2E=2E_{Tot}$, if $L=-\frac{1}{2}\TDy{t}{\Omega}$, $\Omega\propto \frac{1}{R}\propto\rho^{\frac{1}{3}}$ implica $\frac{1}{\Omega}\TDy{t}{\Omega}=\frac{1}{3}\frac{1}{\rho}\TDy{t}{\rho}$     
            \begin{align*}
                &n_e=\frac{\rho}{\mu_em_H}=\frac{8\pi p_F^3}{3h^3}\Rightarrow\rho\proptoE_F^{\frac{3}{2}}\\
...
\begin{frame}{core He-burning of low mass*-Mixinge Proc.: convection where $\nabla_{rad}>\nabla_{ad}$}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
...
\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,height=0.45\textheight]{HBnablaraddiscsc}\label{fig:HBnablaraddiscsc}
\end{figure}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{instability strip: Cepheids and RR Lyrae - standard candles for PopI and PopII}
%Refs: [61], [79], [80]
\begin{columns}[T]
	\begin{column}{0.45\textwidth}
	\begin{itemize}
	\item stable radial pulsation : $\kappa$-mechanism - star energy flow fixes the radius - stability implies variation in energy flux and thus in R - opacity increases during compression/decreases in expansionat same level in stellar envelope: He/H ionization ragions
        \item Boundary of IS - cool boundary: \xdiminuisce{T_e}, convection set in - hot boundary: \xaumenta{T_e}, \xdiminuisce{M_{ion}}, \xaumenta{R_{ion}}
	...
	\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,height=0.50\textheight]{HBPulsation-RRIS}\label{fig:HBPulsation-RRIS}
	\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,height=0.40\textheight]{HBPulsation-CIS}\label{fig:HBPulsation-CIS}
	\end{figure}
\end{column}\end{columns}
\end{frame}
...
\begin{column}{0.40\textwidth}
	...
	\begin{figure}[!ht]
	\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,height=0.50\textheight]{HBPulsating-B}\label{fig:HBPulsating-B}
	\includegraphics[trim={0cm 0cm 1cm 0cm},clip, keepaspectratio,height=0.40\textheight]{HBPulsating-P}\label{fig:HBPulsating-P}
	\end{figure}
\end{column}\end{columns}
\end{frame}
...
\begin{frame}{NSE and Freeze-out}
...
\begin{columns}[T]
	\begin{column}{0.4\textwidth}
		\begin{itemize}
            ...
            \item SN Ia has fews invariants besides maximum bolometric magnitude B:
                \begin{align*}
                    &M_B=-21.726 +2.698\Delta m_{15}(B)\\
                    &M_V=-20.883+1.949\Delta m_{15}(B)\\
                    &M_I=-19.591+1.076\Delta m_{15}(B)
                \end{align*}
                decline rate $\Delta m_{15}$ between tim of maximum and 15 days later.
        \end{itemize}
	\end{column}
	\begin{column}{0.5\textwidth}
		\begin{figure}[!ht]
...
\begin{frame}{Struttura di equilibrio: Caratteristiche e metodo di calcolo}
    $c_b=0.221-0.225$\si{\mega\meter\per\second}, $tsun{}\approx\SI{4.5669}{\year}$, $c^2=\frac{\Gamma_1P}{\rho}$, $\Gamma_1=\TDlny{\rho}{P}$,  $R_b\approx0.710-0716\rsun{}$, $Y_{ph}\approx0.233-0.268$.
\end{frame}
