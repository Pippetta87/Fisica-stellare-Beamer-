...
\begin{columns}[T]
	\begin{column}{0.65\textwidth}
\begin{itemize}
\item General fading of mag with ages for $t>\SI{e7}{\year}$ of \SI{2}{\mag\per\dex}
\item For K band sudden brightening at \SI{200}{\mega\year} due to AGB
...
\item Age-Metallicity degeneracy for $T>\SI{1}{\giga\year}$: $\frac{\Delta t}{t}\approx1.5-2.0\frac{\Delta Z}{Z}$ for \SI{12}{\giga\year} and solar metallicity. C-C diagram: searches for color pair where line of constant Z/t are orthogonal - $(B-K)$ sensitive to age and $(J-K)$ to Z - J/K dominated by AGB ($t<\SI{1}{\giga\year}$) and upper RGB for higher ages (sensitive to Z), B from higher MS and TO (sensitive to age)
\item FAst evolving upper RGB/AGBare main contributors to J and K integrated mag: for small mass of SSP ($\leqe5-e6\msun$ upper und spectrum of stellar cluster) number of stars pop fast evol phases are subjected to fluctuation - this can cause mag colour fluct.
\end{itemize}
	\end{column}
	\begin{column}{0.35\textwidth}
\begin{figure}[!ht]
	\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,height=0.28\textheight]{unresolved-timecolour}
	\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,height=0.28\textheight]{unresolvedBKJK}
	\includegraphics[trim={0cm 0cm 0 0},clip, keepaspectratio,height=0.28\textheight]{unresolved-intmag}
\end{figure}
	\end{column}
\end{columns}
\end{frame}
...
$I_{\angstrom}=\int_{\lmabda_1}^{\lambda_2}(1-\frac{F_{I,\lambda}}{F_{C.\lambda}})\,d\lambda$
